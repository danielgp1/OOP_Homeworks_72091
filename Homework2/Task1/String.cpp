...
{
	if (this->size + _string.size >= this->capacity)
		this->resize();
	for (int i = this->size - 1, j = 0; j < _string.size; i++, j++)
	{
		this->charArray[i] = _string.charArray[j];
	}
	this->size += _string.size - 1;
	return *this;
}
